...
int value = 0;
String variable = "";  // #problem for memory consumption
...
void reconnect() {
  ...
  while (!client.connected()) {
    ...
    if (client.connect(clientId.c_str())) {
      ...
      // Serial.println(ESP.getFreeHeap()); // #problem for memory consumption
      delay(5000);
    }
  }
}
...
void setup() {
  ...
  pinMode(btn1_pin, INPUT_PULLUP);         // #problem for manual reset
  Serial.begin(115200);
  setup_wifi();
...
uint32_t matrix[10000]; // #problem for memory consumption
float matrix2[1000]; // #problem for memory consumption
String matrix3[5]; // #problem for memory consumption

void loop() {

  delay(1);
  matrix[0]++; // #problem for memory consumption
  matrix[9999]++; // #problem for memory consumption
  matrix2[0]++; // #problem for memory consumption
  matrix2[999]++; // #problem for memory consumption
  for (uint8_t i = 0; i < 5; i++){
    matrix3[i] += " "; // #problem for memory consumption
  }
  ...
  if (digitalRead(btn1_pin) == LOW){ // #problem for manual reset
    Serial.println("reset");
    delay(100);
    ESP.restart();
  }

  variable += "                             "; // #problem for memory consumption
}
